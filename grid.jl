...
function Base.floor(grid::LogGrid{T, SIZE, 2}, x) where {T, SIZE}
...
function Base.floor(grid::LogGrid{T, SIZE, 3}, x) where {T, SIZE}
    segment=grid.segment
    if x<grid[2]
        return 1
    elseif x<grid[segment[1]]
...
Base.getindex(grid::LogGrid, i) = grid.grid[i]
Base.firstindex(grid::LogGrid) = 1
Base.lastindex(grid::LogGrid) = grid.size
...
@inline function angle(size, type = Float64)
    return Uniform{type,size}(-1.0, 1.0, [false, false])
end
